...
# NOTE: See `data/README.md` for instructions on setting up the dataset.


def build_dataset(datadir, num_docs=0, max_words=None, vocab_size=None, years=None):
    """
    ...
    if not os.path.exists(speeches_dir):
        ...
        elif os.path.isfile(f"{speeches_dir}.tgz"):
            print("Unzipping speeches.tgz; this will take a minute")
            with tarfile.open(f"{speeches_dir}.tgz") as inf:
                inf.extractall(datadir)
        else:
            print("Couldn't find speeches archive!")
            raise IOError("See `data/README.md` to set up the dataset.")

    files = []
    for fn in os.listdir(speeches_dir):
        if os.path.isfile(os.path.join(speeches_dir, fn)):

            # If we're specifying specific years to include, filter here.
            if years is not None:
                try:
                    if int(fn[:4]) not in years:
                        continue
                except ValueError:
                    continue
    ...
    files = files[len(files)-num_docs:]
    ...
    for i, fn in enumerate(files, start=1):
        ...
        try:
            ...
            print(f"WARNING: Skipping {fn} due to {type(e)}: {str(e)}")
...
def train_test_unlabeled_split(data, labels, speeches, splits):
    ...
    train_speech = np.array(speeches)[split_vector == 0]
    test_speech = np.array(speeches)[split_vector == 1]
...
def clean_text(text, max_words=None, stopwords=None):
    ...
    # skipping first ~30 words, which are often introductory
    if max_words is None:
        return good_tokens[30:]
    else:
        return good_tokens[30:30+max_words]
